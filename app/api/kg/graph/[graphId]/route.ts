...
export async function GET(request: Request, { params }: Params) {
    ...
    return NextResponse.json(
      {
        ...
        error: {
          ...
          message: "Local graph ids are stored in browser sessionStorage and cannot be fetched server-side.",
        },
      },
      { status: 400 }
    );
  ...
  const path = endpoint.pathTemplate.replace(`{${endpoint.idParam}}`, encodedId);
  ...
  const parsed = parsePayload(backendResponse.headers.get("content-type"), rawBody);
    ...
    return NextResponse.json(
      {
        ...
        error: {
          ...
          detail: { backendUrl, status: backendResponse.status, payload: parsed },
        },
      },
      { status: 502 }
    );
